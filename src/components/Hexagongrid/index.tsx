...
const getGridDimensions = (gridWidth: any, gridHeight: any, N: any) => {
...
const tryInvoke = (func: any, params = [], defaultValue = null) => {
  return isFunction(func) ? func(...params) : defaultValue;
};

const HexagonGrid = (props: any) => {
  ...
  const getHexDimensions = (row: any, col: any) => {
    const dimensions = {
  ...
  const getRowDimensions = (row: any) => {
    const dimensions = {
    ...
    if (row % 2 === 0) {
      (dimensions as any).marginLeft = `${(state.hexSize / 2) * 3}px`;
    }
    return dimensions;
  };
        ...
        return (
          <svg key={row} width={rowDim.width} height={rowDim.height} y={rowDim.y}>
            {times(columns, (col) => {
              ...
              const _hexProps = tryInvoke(hexProps, [hexagon] as any, hexProps);
              return (
                <svg key={iHexagon} height={hexDim.height} width={hexDim.width} x={`${hexDim.x}px`}>
                  <Hexagon {..._hexProps} flatTop>
                    {tryInvoke(renderHexagonContent, [hexagon] as any, (<tspan />) as any)}
                  </Hexagon>
                </svg>
              );
            })}

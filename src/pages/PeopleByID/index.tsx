...
const PeopleByID = () => {
  ...
  useEffect(() => {
    if (peoplesList) {
      (async () => {
        ...
        const idsSpecies = peoplesList.species.map((speccy) => speccy.split('/')[5]);
        ...
        const idsStarships = peoplesList.starships.map((starship) => starship.split('/')[5]);
        ...
        const idsVehicles = peoplesList.vehicles.map((vehicle) => vehicle.split('/')[5]);
  ...
  if (!peoplesList || !filmsList || !planetsList || !speciesList || !starshipsList || !vehiclesList) return <Spiner />;

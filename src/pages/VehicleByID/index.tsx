...
const StarshipByID = () => {
  ...
  useEffect(() => {
    if (vehiclesList) {
      (async () => {
        const idsFilms = vehiclesList.films.map((speccy) => speccy.split('/')[5]);
        ...
        const idsPeople = vehiclesList.pilots.map((people) => people.split('/')[5]);
  ...
  if (vehiclesList === null || filmsList === null || peopleList === null) return <Spiner />;
